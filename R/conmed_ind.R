...
# calculating statistics used in every case
  ...
  obs_r_a <- (est_eff_a / std_err_a) / sqrt(((n_obs - n_covariates_a - 2) + ((est_eff_a / std_err_a)^2)))
  obs_r_b <- (est_eff_b / std_err_b) / sqrt(((n_obs - n_covariates_b - 2) + ((est_eff_b / std_err_b)^2)))
  ...
  if (focus != "both a and b") {
    ...
    output_print(ME = F, focus, est_eff, beta_threshold, nu, obs_r, critical_r, r_con, itcv, alpha)
